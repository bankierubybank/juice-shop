                        ...
                        stage('SONATYPE: SCA') {
                            steps {
                                ...
                                dir(env.IMAGE_NAME) {
                                    ...
                                    sh 'npm config set registry https://nexus.devops.demo/repository/npm-proxy/'
                ...
                stage('SONATYPE: SBOM CD') {
                    steps {
                        script {
                            ...
                            env.IQ_APP_ID = sh(script: "(curl -u ${IQ_USERNAME}:${IQ_PASS} -X GET 'https://nexusiq.devops.demo:8443/api/v2/applications?publicId=${IQ_APP_NAME}' -H 'Accept: application/json' | jq -r '.applications[].id')", returnStdout: true).trim()
                            
                            // Generate SBOM file of latest scan
                            env.SBOM = sh (script: "(curl -u ${IQ_USERNAME}:${IQ_PASS} -X GET 'https://nexusiq.devops.demo:8443/api/v2/cycloneDx/1.4/${IQ_APP_ID}/stages/build' -H 'Accept: application/xml' -o ${JOB_BASE_NAME}-${BUILD_NUMBER}-bom.xml)", returnStdout: true).trim()
                        }
                        nexusPolicyEvaluation advancedProperties: '',
                            enableDebugLogging: false,
                            failBuildOnNetworkError: false,
